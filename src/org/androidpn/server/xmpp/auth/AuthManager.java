...
public class AuthManager {
     ...
     * @throws ApnUserNotFoundException if the your was not found
     */
    public static String getPassword(String clientId)
            throws ApnUserNotFoundException {
        return ServiceLocator.getApnUserService().getApnUserByClientId(clientId)
                .getPassword();
    }
     ...
     */
    public static AuthToken authenticate(String clientId, String password)
            throws UnauthenticatedException {
    	System.out.println("AuthManager authenticate..76");
        ...
        if (clientId.contains("@")) {
            ...
            String domain = clientId.substring(index + 1);
        ...
        try {
            if (!password.equals(getPassword(clientId))) {
                throw new UnauthenticatedException();
            }
        } catch (ApnUserNotFoundException unfe) {
            throw new UnauthenticatedException();
        }
     ...
     * @throws UnauthenticatedException if the username and password do not match 
     */
    public static AuthToken authenticate(String clientId, String token,
            String digest) throws UnauthenticatedException {
        if (clientId == null || token == null || digest == null) {
            throw new UnauthenticatedException();
        }
        clientId = clientId.trim().toLowerCase();
        if (clientId.contains("@")) {
