...
public class IQRegisterHandler extends IQHandler {
    ...
    private ApnUserService userService;
    ...
    public IQRegisterHandler() {
        userService = ServiceLocator.getApnUserService();
        ...
        if (IQ.Type.get.equals(packet.getType())) {
            ...
            try {
                ...
                if (query.element("remove") != null) {
                    ...
                    String clientId = query.elementText("clientId");
...
//                    }
                    ...
                    ApnUser user;
                    if (session.getStatus() == Session.STATUS_AUTHENTICATED) {
                        user = userService.getApnUser(session.getUsername());
                    } else {
                        user = new ApnUser();
                    }
...
//                    user.setName(name);
                ...
                reply = IQ.createResultIQ(packet);
                reply.setChildElement(packet.getChildElement().createCopy());
                if (ex instanceof ApnUserExistsException) {
                    reply.setError(PacketError.Condition.conflict);
                } else if (ex instanceof ApnUserNotFoundException) {
